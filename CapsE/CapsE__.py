...
class CapsE(object):
    def __init__(self, 
        ...
        with tf.name_scope("embedding"):
            ...
            self.W_doc = tf.get_variable(name="W_doc", initializer=initialization[1], trainable=False)

        self.embedded_query = tf.nn.embedding_lookup(self.W_query, self.input_x[:, 0])
        self.embedded_doc = tf.nn.embedding_lookup(self.W_doc, self.input_x[:, 1])
        ...
        self.embedded_chars = tf.concat([self.embedded_query, self.embedded_doc], axis=1)
